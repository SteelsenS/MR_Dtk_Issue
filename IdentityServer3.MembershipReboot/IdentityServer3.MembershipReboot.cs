...
namespace IdentityServer3.MembershipReboot
{
        ...
        where TAccount : UserAccount
    {
        ...
        protected virtual IEnumerable<Claim> GetClaimsFromAccount(TAccount account)
        {
            var claims = new List<Claim>
            {
                ...
                new Claim(Constants.ClaimTypes.UpdatedAt,
                    EpochTimeExtensions.ToEpochTime(account.LastUpdated).ToString(),
            ...
            if (!string.IsNullOrWhiteSpace(account.MobilePhoneNumber))
            {
                ...
                claims.Add(new Claim(Constants.ClaimTypes.PhoneNumberVerified,
                    !string.IsNullOrWhiteSpace(account.MobilePhoneNumber) ? "true" : "false"));
            }
        ...
        public override async Task AuthenticateExternalAsync(ExternalAuthenticationContext ctx)
        {
            ...
            try
            {
                var tenant = string.IsNullOrWhiteSpace(message.Tenant)
                    ? userAccountService.Configuration.DefaultTenant
                    : message.Tenant;
                var acct = userAccountService.GetByLinkedAccount(tenant, externalUser.Provider,
                    externalUser.ProviderId);
                if (acct == null)
                {
                    ctx.AuthenticateResult =
                        await
                            ProcessNewExternalAccountAsync(tenant, externalUser.Provider, externalUser.ProviderId,
                                externalUser.Claims);
                }
                else
                {
                    ctx.AuthenticateResult =
                        await
                            ProcessExistingExternalAccountAsync(acct.ID, externalUser.Provider, externalUser.ProviderId,
                                externalUser.Claims);
                }
            }
            catch (ValidationException ex)
        ...
        protected virtual async Task<AuthenticateResult> ProcessNewExternalAccountAsync(string tenant, string provider,
        ...
        protected virtual Task<TAccount> TryGetExistingUserFromExternalProviderClaimsAsync(string provider,
            IEnumerable<Claim> claims)
        {
            return Task.FromResult<TAccount>(null);
        }

        protected virtual Task<TAccount> InstantiateNewAccountFromExternalProviderAsync(string provider,
            string providerId, IEnumerable<Claim> claims)
        {
        ...
        protected virtual async Task<AuthenticateResult> SignInFromExternalProviderAsync(Guid accountID, string provider)
        {
            ...
            return new AuthenticateResult(accountID.ToString("D"), GetDisplayNameForAccount(accountID), claims, provider,
                Constants.AuthenticationMethods.External);
        }

        protected virtual Task<AuthenticateResult> UpdateAccountFromExternalClaimsAsync(Guid accountID, string provider,
            string providerId, IEnumerable<Claim> claims)
        {
